...
async def translate(target_lang: str, text: List[str] = Query([]), source_lang: Optional[str] = '',
    ...
    if not IS_BACKEND:
        ...
        data = {'target_lang': target_lang, 'text': text, 'source_lang': source_lang, 'beam_size': beam_size,
        ...
        if beam_size < 1 or (
                'EASYNMT_MAX_BEAM_SIZE' in os.environ and beam_size > int(os.getenv('EASYNMT_MAX_BEAM_SIZE'))):
        ...
        try:
            output['translated'] = model.translate(text, target_lang=target_lang, source_lang=source_lang,
                                                   beam_size=beam_size,
                                                   perform_sentence_splitting=perform_sentence_splitting,
